...
public class FullPrunedBlockChain extends AbstractBlockChain
{
     ...
     */
    public FullPrunedBlockChain(Context context, Wallet wallet, FullPrunedBlockStore blockStore)
     ...
     * one from scratch, or you can deserialize a saved wallet from disk using {@link Wallet#loadFromFile(java.io.File)}.
     */
    public FullPrunedBlockChain(NetworkParameters params, Wallet wallet, FullPrunedBlockStore blockStore)
        throws BlockStoreException
    {
     ...
     */
    public FullPrunedBlockChain(Context context, FullPrunedBlockStore blockStore)
        throws BlockStoreException
    {
        this(context, new ArrayList<Wallet>(), blockStore);
    }
     ...
     */
    public FullPrunedBlockChain(Context context, List<Wallet> listeners, FullPrunedBlockStore blockStore)
        throws BlockStoreException
    {
     ...
     */
    public FullPrunedBlockChain(NetworkParameters params, List<Wallet> listeners, FullPrunedBlockStore blockStore)
        throws BlockStoreException
    {
    ...
    @Override
    protected void rollbackBlockStore(int height)
        throws BlockStoreException
    {
        ...
        throws VerificationException, BlockStoreException, PrunedException
    {
        ...
        try
        {
            ...
            if (transactions != null)
            {
                ...
                {
                    final Set<VerifyFlag> verifyFlags = params.getTransactionVerificationFlags(newBlock.getHeader(), tx, getVersionTally(), Integer.SIZE);
                    boolean isCoinBase = tx.isCoinBase();
                    Coin valueIn = Coin.ZERO;
                    Coin valueOut = Coin.ZERO;
                    final List<Script> prevOutScripts = new LinkedList<>();
                    ...
                    if (!isCoinBase)
                    {
                        for (int index = 0; index < tx.getInputs().size(); index++)
                        {
                            ...
                            final UTXO prevOut = blockStore.getTransactionOutput(in.getOutpoint().getHash(), in.getOutpoint().getIndex());
                            if (prevOut == null)
