...
public class TransactionInput extends ChildMessage
{
    ...
    public TransactionInput(NetworkParameters params, @Nullable Transaction parentTransaction, byte[] scriptBytes, TransactionOutPoint outpoint, @Nullable Coin value)
    {
        ...
        length = 40 + (scriptBytes != null ? VarInt.sizeOf(scriptBytes.length) + scriptBytes.length : 1);
    }
     ...
     */
    public TransactionInput(NetworkParameters params, @Nullable Transaction parentTransaction, byte[] payload, int offset)
        ...
        throws ScriptException
    {
        ...
        Script script = (scriptSig != null) ? scriptSig.get() : null;
        if (script == null)
        {
    ...
    void setScriptBytes(byte[] scriptBytes)
    {
        ...
        int newLength = 40 + (scriptBytes != null ? VarInt.sizeOf(scriptBytes.length) + scriptBytes.length : 1);
    ...
    TransactionOutput getConnectedOutput(Map<Sha256Hash, Transaction> transactions)
    {
        ...
        return (tx != null) ? tx.getOutputs().get((int)outpoint.getIndex()) : null;
    }
    ...
    {
        Transaction tx = transactions.get(outpoint.getHash());
        return (tx != null) ? connect(tx, mode) : TransactionInput.ConnectionResult.NO_SUCH_TX;
    }
    ...
    public String toString()
    {
        try
        {
            ...
            else
            {
                ...
                String flags = Joiner.on(", ").skipNulls().join(hasSequence() ? "sequence: " + Long.toHexString(sequence) : null, isOptInFullRBF() ? "opts into full RBF" : null);
                if (!flags.isEmpty())
