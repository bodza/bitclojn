...
public class Block extends Message
{
     ...
     */
    protected void parseTransactions(final int transactionsOffset)
        throws ProtocolException
    {
        ...
        throws IOException
    {
        ...
        if (headerBytesValid && payload != null && offset + HEADER_SIZE <= payload.length)
        {
    ...
    private void writeTransactions(OutputStream stream)
        throws IOException
    {
        ...
        if (transactionBytesValid && payload != null && offset + length <= payload.length)
        {
    ...
    public String toString()
    {
        ...
        String bips = Joiner.on(", ").skipNulls().join(isBIP34() ? "BIP34" : null, isBIP66() ? "BIP66" : null, isBIP65() ? "BIP65" : null);
        if (!bips.isEmpty())
        ...
        if (transactions != null && 0 < transactions.size())
        {
     ...
     * @throws VerificationException if there was an error verifying the block.
     */
    public void verifyTransactions(final int height, final EnumSet<VerifyFlag> flags)
        throws VerificationException
    {
     ...
     * @throws VerificationException if there was an error verifying the block.
     */
    public void verify(final int height, final EnumSet<VerifyFlag> flags)
        throws VerificationException
    {
        verifyHeader();
    ...
    {
        return createNextBlock(null, version, (TransactionOutPoint)null, Utils.currentTimeSeconds(), pubKey, FIFTY_COINS, height);
    }
