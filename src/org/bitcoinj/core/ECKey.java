...
public class ECKey implements EncryptableItem
{
    ...
    protected ECKey(@Nullable BigInteger priv, LazyECPoint pub)
    {
        if (priv != null)
        {
            checkArgument(priv.bitLength() <= (32 << 3), "private key exceeds 32 bytes: {} bits", priv.bitLength());
    ...
    {
        this((privKeyBytes != null) ? new BigInteger(1, privKeyBytes) : null, pubKey);
    }
     ...
     */
    public void verifyOrThrow(byte[] hash, byte[] signature)
     ...
     * @throws java.security.SignatureException if the signature does not match.
     */
    public void verifyOrThrow(Sha256Hash sigHash, ECDSASignature signature)
        throws SignatureException
    {
    ...
    private static ECKey extractKeyFromASN1(byte[] asn1privkey)
    {
        ...
        try
        {
            ...
            checkArgument(2 <= encoding && encoding <= 4, "Input has 'publicKey' with invalid encoding");
     ...
     * @return encryptedKey
     */
    public ECKey encrypt(KeyCrypter keyCrypter, KeyParameter aesKey)
        throws KeyCrypterException
    {
     ...
     */
    public ECKey decrypt(KeyCrypter keyCrypter, KeyParameter aesKey)
        throws KeyCrypterException
    {
        checkNotNull(keyCrypter);
     ...
     * @param aesKey The KeyParameter with the AES encryption key (usually constructed with keyCrypter#deriveKey and cached).
     */
    public ECKey decrypt(KeyParameter aesKey)
        throws KeyCrypterException
    {
     ...
     */
    public ECKey maybeDecrypt(@Nullable KeyParameter aesKey)
        throws KeyCrypterException
    {
    ...
    {
        return (keyCrypter != null && encryptedPrivateKey != null && 0 < encryptedPrivateKey.encryptedBytes.length);
    }
    ...
    public void formatKeyWithAddress(boolean includePrivateKeys, StringBuilder sb, NetworkParameters params)
    {
